...
export function TimelinePage() {
  ...
  const handleItemClick = useCallback(
    (item: db.TimelineEvent) => navigate(`/timeline/${item.id}`),
    [navigate],
  );

  const handleTabChange = useCallback(
    (tab: string) => {
      if (tab === "list") navigate("/timeline");
      else if (tab === "detail")
        navigate(`/timeline/${filteredItems[0]?.id || 1}`);
    },
  ...
  return (
    <NavigationProvider
      onNavigate={(entityType, navId) => {
        if (entityType === "timeline") navigate(`/timeline/${navId}`);
      }}
    >
      <TabView
        tabs={tabs}
